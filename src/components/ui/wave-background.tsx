...
export function Waves({
  ...
  useEffect(() => {
    ...
    let numNodes = Math.floor(width / NODE_SPACING_PX) + 1
    let numLines = Math.floor(height / LINE_SPACING_PX) + 1 + Math.ceil(PEAK_HEIGHT_PX / LINE_SPACING_PX)
    let lines = Array.from({ length: numLines }, () => new Int16Array(numNodes))
    let linePeaks = Array.from({ length: numLines }, () => new Uint16Array(numNodes))
    let hTotal = numLines * LINE_SPACING_PX
    ...
    const resizeObserver = new ResizeObserver((entries) => {
      for (const entry of entries) {
        ...
        numLines = Math.floor(height / LINE_SPACING_PX) + 1 + Math.ceil(PEAK_HEIGHT_PX / LINE_SPACING_PX)
    ...
    const step = () => {
      ...
      for (let lineIdx = 0; lineIdx < numLines; ++lineIdx) {
        ...
        const peakNodesArr = linePeaks[adjustedLineIdx]
        const isHighlighted = peakNode.y === lineIdx

        if (isHighlighted && !peakNodesArr[peakNode.x]) {
          let newAge = 1
          ...
          peakNodesArr[peakNode.x] = newAge
        }
        ...
        for (let nodeIdx = 0; nodeIdx < numNodes; ++nodeIdx) {
          ...
          for (let peakNodeIdx = 0; peakNodeIdx < numNodes; ++peakNodeIdx) {
            const age = peakNodesArr[peakNodeIdx] * speed
            if (age) {
        ...
        for (let peakNodeIdx = 0; peakNodeIdx < numNodes; ++peakNodeIdx) {
          if (peakNodesArr[peakNodeIdx]) ++peakNodesArr[peakNodeIdx]
        }
      }
  ...
  }, [backgroundColor, foregroundColor, speed, amplitude, peakHeight])
